// Louisiana Civil Code corpus loader.
//
// Reads from `tmp/corpus/usufruct-<tag>/` (root of the release zip).
...
// Re-export the shared status type under the legacy name some pages use.
...
export const ccManifest: CcManifest = readJSON<CcManifest>('manifest.json');

// Legacy export name; older importers may still reference `manifest`.
export const manifest = ccManifest;
...
function buildContainers(): Container[] {
  ...
  function walk(node: TreeNode, ancestors: HierStep[]): Container {
    ...
    const c: Container = {
      ...
      url: containerPath('/cc', ancestors, self),
      children: [],
...
// Re-export ACTIVE_TAG for backward compatibility — some pages import it
// from corpus.ts directly, but per-corpus modules forward it for parity.
export { ACTIVE_TAG };
